...
namespace ZoomFileManager.Services.Providers
{
    ...
    public class OnedriveUserProvider : IUploadService
    {
        ...
        private GraphServiceClient GetTargetClient(string name)
        {
            if (!_clients.ContainsKey(name))
                _clients.Add(name, new GraphServiceClient(OnedriveHelpers.DoAuth(
                    (OD_UserClientConfig)(_targetConfigs[name]?.ClientConfig ??
        ...
        private async Task<UploadResult<DriveItem>> PerformUpload(UploadSession uploadSession, Stream sourceFileStream,
        ...
        public async Task<UploadResult<DriveItem>> PutFileAsync(IFileInfo sourceFileInfo, string? relativePath,
