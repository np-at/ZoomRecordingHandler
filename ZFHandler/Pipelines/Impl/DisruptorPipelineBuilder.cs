...
namespace ZFHandler.Pipelines.Impl
{
     public class DisruptorPipelineBuilder : IPipelineBuilder
    {
        ...
        public interface IStep : IWorkHandler<DisruptorEvent>, IEventHandler<DisruptorEvent>
        {
        ...
        public class PipelineContainerOptions
        {
            public List<IStep> Steps { get; set; }
        }
        public class PipelineContainer<T> : INotificationHandler<T> where T : INotification
        {
            private readonly IServiceProvider _serviceProvider;
            private readonly PipelineContainerOptions _options;
            public PipelineContainer(IServiceProvider serviceProvider, IOptions<PipelineContainerOptions> options)
            {
            ...
            {
                var disruptor = new Disruptor<DisruptorEvent>(() => new DisruptorEvent(), 1024, TaskScheduler.Default,
                    ProducerType.Multi, new BlockingSpinWaitWaitStrategy());
                var firstStep = _options.Steps.First();
                var currentGroup = firstStep.IsWorkerPool
                    ? disruptor.HandleEventsWithWorkerPool(firstStep.AsWorkerPool())
                    : disruptor.HandleEventsWith(firstStep);

                foreach (var step in _options.Steps.Skip(1))
                {
                    currentGroup = step.IsWorkerPool
                        ? currentGroup.HandleEventsWithWorkerPool(step.AsWorkerPool())
                        : currentGroup.HandleEventsWith(step);
                }
                // currentGroup.HandleEventsWith(new Releaser(), );
